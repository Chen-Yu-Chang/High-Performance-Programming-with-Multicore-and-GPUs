...
//     A "timespec" is a structure that looks like this:
//
//           struct timespec {
...
int main(int argc, char ** argv)
{
    ...
    /* Now we have a number of loop iterations that takes between 1 and 2
     seconds to run. Next we need to delay a little to make sure the
     other copies of this benchmark all have a chance to do the same thing. */
    bench1(array_size, loop_iters);
    
    /* Now run the test again, this is the measurement we actually keep. */
    bench_time = bench1(array_size, loop_iters);
    
    /* If we finished more sooner than the other tasks, we want to make sure
     the computer is equally busy while they're doing their measurement.
     So we run the test one more time to keep CPU load high. */
    bench1(array_size, loop_iters);
    ...
    /* Compute bytes/sec and print the result. Size of memory allocation
     ...
     divided by time in seconds = bytes/sec */
