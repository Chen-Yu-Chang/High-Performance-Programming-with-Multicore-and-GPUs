...
int main(int argc, char *argv[])
{
    ...
    OPTION++;
    printf("OPTION=%d (serial SOR_OMP)\n", OPTION);
    for (x=0; x<NUM_TESTS && (n = A*x*x + B*x + C, n<=alloc_size); x++) {
        printf("  iter %d rowlen = %d\n", x, GHOST+n);
        init_array_rand(v0, GHOST+n);
        set_arr_rowlen(v0, GHOST+n);
    ...
    printf("\n");
    printf("size, SOR time, SOR iters, OMP time, OMP iters, SOR_ji time, SOR_ji iters, SOR_blocked time, SOR_blocked iters\n");
    {
        int i, j;
...
#pragma omp parallel shared(data, length, total_change, iters, change) private(i, j)
    {
        while((total_change/(double)(length*length)) > (double)TOL) {
            iters++;
            total_change = 0;
